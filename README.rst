########################################
Building and uploading pygraphviz wheels
########################################
...
https://travis-ci.org/matthew-brett/pygraphviz-wheels

Appveyor interface will be at
https://ci.appveyor.com/project/pygraphviz/pygraphviz-wheels
...
* builds a pygraphviz wheel, linking against these fresh builds;
...
* making a commit to the ``pygraphviz-wheels`` repository (e.g. with ``git commit
  --allow-empty``); or
...
Which pygraphviz commit does the repository build?
====================================================

The ``pygraphviz-wheels`` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -u $CDN_URL -s -v -w ~/wheelhouse -t all pygraphviz $VERSION
...
``pygraphviz`` is the root name of the wheel(s) to download / upload, and
...
`pygraphviz-1.4-` from the URL in ``$CDN_URL`` above to ``~/wheelhouse``, then
upload them to PyPI.
